...
Zodra `scrollLeft > 0` op de tabelwrapper: voeg een rechts-gerichte shadow toe aan de actiemenu-cel (`.sticky-first`):

```css
.table-wrap[data-scrolled="true"] .sticky-first {
...
interface Column {
  ...
  sticky?: boolean            // sticky positie (actiemenu kolom)
  stickyLeft?: number         // left-offset in px voor left-sticky kolommen
